...
sims.dat$fx[sims.dat$model == "RFX"] = "blk:tt"
rm(tmp)
...
ffxint.d.p <- plt.fx.sz(sims.dat[sims.dat$model == "FFX" & sims.dat$fx == "blk:tt", ], ylims)
#rfxint.d.p <- plt.fx.sz(sims.dat[sims.dat$model == "RFX" & sims.dat$fx == "me", ], ylims)
rfxme.d.p <- plt.fx.sz(sims.dat[sims.dat$model == "RFX" & sims.dat$fx == "blk:tt", ], ylims)

# now for p-values
#ffxme.p.p <- plt.ps(sims.dat[sims.dat$model=="FFX" & sims.dat$fx == "me",], c(0,1))
ffxint.p.p <- plt.ps(sims.dat[sims.dat$model=="FFX" & sims.dat$fx == "blk:tt",], c(0,1))
#rfxint.p.p <- plt.ps(sims.dat[sims.dat$model=="RFX" & sims.dat$fx == "me",], c(0,1))
rfxme.p.p <- plt.ps(sims.dat[sims.dat$model=="RFX"& sims.dat$fx == "blk:tt",], c(0,1))
...
p = plot_grid(ffxint.d.p, rfxme.d.p, ffxint.p.p, rfxme.p.p, labels=c('A', 'B', 'C', 'D'), label_size = 12, align="v")
p = p + ggsave(paste(plot.fname, ".png", sep=""), width = width, height = height, units="in")
