...
dat = dat %>% pivot_longer(c('longLag', 'shortLag'), names_to = "block", values_to="RT") # move to a tidy frame
...
# ----------------------------------------------------------------------------------------------------
# convert dat from wide to long
dat <- dat %>% pivot_longer(c('lag_2', 'lag_3', 'lag_5', 'lag_7'), names_to = "lag", values_to="acc")
...
# run simulations, getting p values from t.tests, and cohen's d values, and save results to a list
subs = unique(dat$Subjects)
sims = replicate(n.perms, lapply(sub.Ns, function(x) run.aov.AB.sim(data=dat, subs=subs,
                                                                    N=x)), simplify = FALSE)
