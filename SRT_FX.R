...
##### --------------------------------------------------------
task.order.srt <- dat %>% group_by(Subj.No, Task.Order, Block.No, Block.No.Names) %>% 
                          filter(RT.ms > 200) %>%
                          filter(RT.ms < mean(RT.ms) + 3*sd(RT.ms)) %>%
...
t.o <- task.order.srt %>% ggplot(aes(x=as.factor(Block.No), y=mean, fill=Block.No.Names)) + ylim(200,1000) +
      geom_boxplot() +
...
experimenter.srt <- dat %>% group_by(Experimenter, Trial.Type) %>% 
                            filter(RT.ms > 200) %>%
                            filter(RT.ms < mean(RT.ms) + 3*sd(RT.ms)) %>%  
                            summarise(mean=mean(RT.ms))
...
ex <- experimenter.srt %>% ggplot(aes(x=Trial.Type, y=mean, group=Experimenter)) + ylim(200,750) +
  geom_line() +
